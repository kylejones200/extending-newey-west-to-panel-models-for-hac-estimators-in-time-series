...
panel_model = PanelOLS(data["Oil"], X_matrix, entity_effects=True).fit(cov_type="kernel", kernel="bartlett", bandwidth=3)
...
panel_model_clustered = PanelOLS(data["Oil"], X_matrix, entity_effects=True).fit(cov_type="clustered", cluster_entity=True)
